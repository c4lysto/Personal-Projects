...
#error VEC4V ACCESSORS NOT DEFINED
	...
	__forceinline void SetXYZ(Vec3V_In rhs) {*this = Vec4V(rhs, w);}
	__forceinline void SetXYZ(Vec3V&& rhs) {*this = Vec4V(rhs, w);}
...
{
	return Vec4V(_mm_sub_ps(row, vVector.row));
}
...
__forceinline Vec4V Vec4V::operator+(Vec4V_In vVector) const
{
	return Vec4V(_mm_add_ps(row, vVector.row));
}
...
{
	return Vec4V(_mm_div_ps(row, _mm_set1_ps(fScalar)));
}
...
__forceinline Vec4V Vec4V::operator*(Vec4V_In vVector) const
{
	return Vec4V(_mm_mul_ps(row, vVector.row));
}
...
__forceinline Vec4V Vec4V::operator*(float fScalar) const
{
	return Vec4V(_mm_mul_ps(row, _mm_set1_ps(fScalar)));
}
...
{
	return Vec4V(_mm_mul_ps(vVector.row, _mm_set1_ps(fScalar)));
}
