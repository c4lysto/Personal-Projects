...
const UINT g_dirLightConstByteWidth = (sizeof(DirectionalLight) + (16 - (sizeof(DirectionalLight) % 16)));
...
void DirectionalLight::UpdateConstantBuffer(ID3D11DeviceContext* pContext)
{
	if(pContext)
	{
		...
		pContext->Map(m_pDirLightConstBuffer, NULL, D3D11_MAP_WRITE_DISCARD, 0, &lightSubresource);
			...
			memcpy(lightSubresource.pData, &m_vColor, sizeof(DirectionalLight));
