...
struct Matrix4f
{
	...
	inline float operator[](size_t ucIndex) const { return m[ucIndex]; }
	...
	inline Matrix4f& Scale(const Vec3f& vScale) {return Scale(vScale.x, vScale.y, vScale.z);}
	...
	inline Matrix4f& SetScale(const Vec3f& vScale) {return SetScale(vScale.x, vScale.y, vScale.z);}
	...
	inline Matrix4f& Translate(Vec3f vTranslation) {position += vTranslation; return *this;}

	inline Matrix4f& MoveForward(float fMovement) {position += zAxis * fMovement; return *this;}
	inline Matrix4f& MoveBackward(float fMovement) {position -= zAxis * fMovement; return *this;}
	inline Matrix4f& MoveLeft(float fMovement) {position -= xAxis * fMovement; return *this;}
	inline Matrix4f& MoveRight(float fMovement) {position += xAxis * fMovement; return *this;}
	inline Matrix4f& MoveUp(float fMovement) {position += yAxis * fMovement; return *this;}
	inline Matrix4f& MoveDown(float fMovement) {position -= yAxis * fMovement; return *this;}

	inline Matrix4f& Transpose() {return *this = XMMatrixTranspose(XMLoadFloat4x4((XMFLOAT4X4*)this));}
	...
	inline Matrix4f& Invert() {return *this = XMMatrixInverse(NULL, XMLoadFloat4x4((XMFLOAT4X4*)this));}
	...
	inline friend Matrix4f MatrixTranspose(const Matrix4f& mMatrix)
	{
	...
	inline friend Matrix4f MatrixTranspos3x3(const Matrix4f& mMatrix)
	{
		Matrix4f tmp(mMatrix);
	...
	inline friend Matrix4f MatrixScale(const Matrix4f& mMatrix, float fXScale, float fYScale, float fZScale)
	{
		Matrix4f tmp(mMatrix);
	...
	inline friend Matrix4f MatrixInverse(const Matrix4f& mMatrix)
	{
		Matrix4f tmp(mMatrix);
	...
	inline friend Matrix4f Lerp(const Matrix4f& MatrixA, const Matrix4f& MatrixB, float fLambda)
	{
		Matrix4f result;
